...
public class MapGenerator : MonoBehaviour
{
			...
			for (int jj = -2; jj < 3; jj++)
				for (int ii = -2; ii < 3; ii++)
					if ((Mathf.Abs(ii) + Mathf.Abs(kk - 5) + Mathf.Abs(jj) < 5))
						chunkData.SetBlock(i + ii, j + jj, k + kk, BlockType.Leaves);

		// Create Trunk
		chunkData.SetBlock(i, j, k, BlockType.Wood);
		chunkData.SetBlock(i, j, k + 1, BlockType.Wood);
		chunkData.SetBlock(i, j, k + 2, BlockType.Wood);
		chunkData.SetBlock(i, j, k + 3, BlockType.Wood);
		chunkData.SetBlock(i, j, k + 4, BlockType.Wood);
		chunkData.SetBlock(i, j, k + 5, BlockType.Wood);
	}
	void GenerateGround(ChunkData chunkData)
	{
	...
	void GenerateTree(ChunkData chunkData)
	{
		for (int j = 0; j < Chunk.ChunkSize; j++)
			for (int i = 0; i < Chunk.ChunkSize; i++)
				for (int k = Chunk.ChunkHeight - 1; k >= 0; k--)
				{
					...
					if (chunkData.Blocks[i, j, k].Type == BlockType.Grass)
					{
						...
						if (treeHeight > treeProbability)
							CreateTree(chunkData, i, j, k + 1, blockPosition);
	...
	public ChunkData GenerateChunkData(ChunkKey chunkKey)
	{
		...
		GenerateGround(chunkData);

		GenerateTree(chunkData);
	...
	void CreateTreeOld(ChunkData chunkData, int i, int j, int k, Vector3 blockPosition)
	{
		var treeHeight = TreeHeightMap.GetHeight(chunkData.WorldPosition.x + blockPosition.x, chunkData.WorldPosition.z + blockPosition.z);
		if (treeHeight > treeProbability)
		{
	...
	public ChunkData GenerateChunkDataOld(ChunkKey chunkKey)
	{
		ChunkData chunkData = new ChunkData();
		chunkData.WorldPosition = new Vector3(chunkKey.x, 0.0f, chunkKey.y) * Chunk.ChunkSize;
			...
			for (int i = 0; i < Chunk.ChunkSize; i++)
			{
				...
				for (int k = 0; k < Chunk.ChunkHeight; k++)
				{
					...
					if (checkTree && blockType == BlockType.Air)
					{
						...
						CreateTreeOld(chunkData, i, j, k, blockPosition);
					}
				}
			}
